...
public final class Constants {
	...
	static {
		...
		ERROR_PRESET = HTML_PRESET.replaceAll("%head%", "<title>%error_type%</title>").replaceAll("%bodyparams%", "style=\"font-family: courier, fixed, monospace;\"").replaceAll("%body%",
				"<center><div style=\"font-size: 50px; margin-top: 60px;\"><strong>%error_type%</strong></div><br>"
						+ "<br><br><div style=\"font-size: 30px; font-family: courier, fixed, monospace;\">%message_small%</div><hr><div style=\"font-size: 18px\">Please contact the server admin: <a href=\"mailto:" + SERVERADMINMAIL + "\">" + SERVERADMINMAIL + "</a><br>Java HTTPServer " + VERSION + "</div></center>");
		...
		PAYMENT_REQUIRED = new FinalResponseImpl(getDefaultHeaders(),
				ERROR_PRESET.replaceAll("%error_type%", "Error - 402").replaceAll("%message_small%", "Payment required!"),
		...
		INTERNAL_ERROR = new FinalResponseImpl(getDefaultHeaders(), ERROR_PRESET.replaceAll("%error_type%", "Error - 500")
				.replaceAll("%message_small%", "An internal Error occoured!"), HTTPResponseCode.INTERNAL_ERROR);
		NOT_IMPLEMENTED = new FinalResponseImpl(getDefaultHeaders(), ERROR_PRESET.replaceAll("%error_type%", "Error - 501")
				.replaceAll("%message_small%", "Not implemented yet!"), HTTPResponseCode.NOT_IMPLEMENTED);
		BAD_GATEWAY = new FinalResponseImpl(getDefaultHeaders(),
				ERROR_PRESET.replaceAll("%error_type%", "Error - 502").replaceAll("%message_small%", "Bad Gateway!"),
				HTTPResponseCode.BAD_GATEWAY);
		SERVICE_UNAVIABLE = new FinalResponseImpl(getDefaultHeaders(), ERROR_PRESET
				.replaceAll("%error_type%", "Error - 503").replaceAll("%message_small%", "Service currently unaviable!"),
				HTTPResponseCode.SERVICE_UNAVIABLE);
		GATEWAY_TIMEOUT = new FinalResponseImpl(getDefaultHeaders(),
				ERROR_PRESET.replaceAll("%error_type%", "Error - 504").replaceAll("%message_small%", "Gateway timeout!"),
